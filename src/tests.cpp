...
void test_shortest_path(Converter adjacency_list_converter,
                        Converter adjacency_matrix_converter,
                        Converter edge_list_converter, int start, int finish,
                        std::pair<long long, std::vector<int>> expected,
                        std::pair<long long, std::vector<int>> (*algorithm)(
                            int start, int finish, Converter converter)) {
...
void test_floyd_warshall(
    ...
    std::vector<std::vector<std::pair<long long, std::vector<int>>>>
        &expected_mtx) {
...
TEST_CASE("Converter", "[Converter]") {
  ...
  SECTION("adjacency matrix") {
    ...
    Graph_ expected = {{{1, 10}, {2, 20}, {4, 50}},
                       {{0, 10}, {3, 30}},
                       {{0, 20}, {3, 40}, {4, 60}},
                       {{1, 30}, {2, 40}, {4, 70}},
                       {{0, 50}, {2, 60}, {3, 70}}};
  ...
  SECTION("edge list (undirected)") {
    std::vector<Edge> edge_list = {{0, 1, 10}, {0, 2, 20}, {0, 4, 50},
                                   {1, 3, 30}, {2, 3, 40}, {2, 4, 60},
                                   {3, 4, 70}};
...
TEST_CASE("Correctness", "[Requirement 1]") {
  SECTION("empty graph") {
    ...
    // Edge list is empty, so inner assertion fails and crushes the program.
    // Can't be "checked" normally.
    ...
    SECTION("Naive Shortest Path") {
      auto res1 = naive_shortest_path(0, 2, adjacency_list_converter);
      auto res2 = naive_shortest_path(0, 2, adjacency_matrix_converter);
  ...
  SECTION("full graph") {
    ...
    SECTION("Dijkstra for high density") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 0, 3, expected,
                         dijkstra_high_density);
    }
    SECTION("Dijkstra for low density") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 0, 3, expected,
                         dijkstra_low_density);
    }
    SECTION("Bellman-Ford") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 0, 3, expected,
                         bellman_for_two_vertices);
    }
    SECTION("Floyd-Warshall") {
      std::vector<std::vector<std::pair<long long, std::vector<int>>>>
          expected_mtx(n);
      ...
      test_floyd_warshall(adjacency_list_converter, adjacency_matrix_converter,
                          edge_list_converter, expected_mtx);
    }

    // Edges have the same weight = 1, everything works correctly
    SECTION("BFS") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 0, 3, expected, bfs);
    }
  ...
  SECTION("complex graph") {
    ...
    std::vector<Edge> edge_list = {{0, 1, 100}, {0, 4, 10}, {0, 3, 20},
                                   {1, 2, 7},   {2, 3, 20}, {3, 4, 15}};
    ...
    SECTION("Dijkstra for high density") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 0, 1, expected,
                         dijkstra_high_density);
    }
    SECTION("Dijkstra for low density") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
    ...
    SECTION("Floyd-Warshall") {
      ...
      expected_mtx[0] = {{0, {0}},
                         ...
                         {20, {0, 3}},
                         {10, {0, 4}}};
      expected_mtx[1] = {{47, {1, 2, 3, 0}},
                         {0, {1}},
                         {7, {1, 2}},
                         {27, {1, 2, 3}},
                         {42, {1, 2, 3, 4}}};
      expected_mtx[2] = {{40, {2, 3, 0}},
                         {7, {2, 1}},
                         {0, {2}},
                         {20, {2, 3}},
                         {35, {2, 3, 4}}};
      expected_mtx[3] = {
          {20, {3, 0}}, {27, {3, 2, 1}}, {20, {3, 2}}, {0, {3}}, {15, {3, 4}}};
      expected_mtx[4] = {{10, {4, 0}},
                         {42, {4, 3, 2, 1}},
                         {35, {4, 3, 2}},
                         {15, {4, 3}},
                         {0, {4}}};
    ...
    SECTION("BFS") {
      ...
      std::vector<Edge> edge_list = {{0, 1, 1}, {0, 4, 1}, {0, 3, 1},
                                     {1, 2, 1}, {2, 3, 1}, {3, 4, 1}};
      ...
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 0, 1, expected, bfs);
    }
  ...
  SECTION("two components") {
    ...
    SECTION("Dijkstra for high density") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 2, 4, expected,
                         dijkstra_high_density);
    }
    SECTION("Dijkstra for low density") {
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
    ...
    SECTION("Floyd-Warshall") {
      std::vector<std::vector<std::pair<long long, std::vector<int>>>>
          expected_mtx(n);
      expected_mtx[0] = {
          {0, {0}}, {-1, {}}, {-1, {}}, {20, {0, 3}}, {10, {0, 4}}};
      expected_mtx[1] = {{-1, {}}, {0, {1}}, {7, {1, 2}}, {-1, {}}, {-1, {}}};
      expected_mtx[2] = {{-1, {}}, {7, {2, 1}}, {0, {2}}, {-1, {}}, {-1, {}}};
      expected_mtx[3] = {
          {20, {3, 0}}, {-1, {}}, {-1, {}}, {0, {3}}, {15, {3, 4}}};
      expected_mtx[4] = {
          {10, {4, 0}}, {-1, {}}, {-1, {}}, {15, {4, 3}}, {0, {4}}};
    ...
    SECTION("BFS") {
      ...
      test_shortest_path(adjacency_list_converter, adjacency_matrix_converter,
                         edge_list_converter, 2, 4, expected, bfs);
    }
