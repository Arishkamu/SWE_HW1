...
bfs(int start, int finish, Converter converter) {
    /*
    * algorithm is used to find the shortest path between start and finish in an unweighted graph
...
std::vector<std::pair<int, int>> lee_algorithm(int start, int finish, Converter converter) {
    /* Algorithm for finding the shortest path between two vertices in a planar unweighted graph.
     * start -- start vertex of path
     * finish -- end vertex of path
     */
    const Graph_& graph = converter.graph;
    int n = graph.size();
